 ...
 * @author Joe-Xidu
 */
@WebServlet(name = "VotersAllSave", urlPatterns = {"/mantenedores/votantes"})
public class VotersAllSave extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        ArrayList<Voter> list = new Gson().fromJson( this.port.all(), java.util.ArrayList.class );
        ...
        view("/maintainers/voters/all.jsp", request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (val.validaterut(rut + '-' + dv)) {
                Voter voter = new Voter(id, rut, dv, name, id_census, status);
                if (id.equalsIgnoreCase("0")) {
                    if (this.port.insert(new Gson().toJson( voter )))
                        json = "{\"response\":1}";
                } else {
                    if (this.port.update(new Gson().toJson( voter )))
                        json = "{\"response\":1}";
                }
            }
