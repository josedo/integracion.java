 ...
 * @author Joe
 */
@WebServlet(name = "StartProcessServlet", urlPatterns = {"/configuracion/comenzar/proceso"})
public class StartProcessServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (process.getStatus().equalsIgnoreCase("1")) {
                if (this.portProcesses.activate(id_process))
                    json = "{\"response\":1}";
            } else
                json = "{\"response\":0, \"msg\":\"Sólo se puede comenzar un proceso activo.\"}";
        } catch (Exception e) {
