                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <table id="mantenedor" class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    <c:forEach items="${list}" var="vocal">
                                        <tr>
                                            <td>${vocal.name}</td>
                                            <td>${vocal.rut} - ${vocal.dv}</td>
                                            <td>${vocal.id_census}</td>
                                            <td><span class="label label-${vocal.status==1?"success":"danger"}">${vocal.status==1?"Activo":"Inactivo"}</span></td>
                                            <td>
                                                <a class="btn btn-primary btn-xs btnEditar" data-id="${vocal.id}" data-url="./vocales/update" data-original-title="Editar" data-toggle="tooltip"><i class="fa fa-pencil-square-o"></i></a>
                                                <c:if test="${vocal.status==1}"><a class="btn btn-primary btn-xs btnEliminar" data-id="${vocal.id}" data-url="./vocales/update" data-original-title="Eliminar" data-toggle="tooltip"><i class="fa fa-times-circle"></i></a></c:if>
                                            </td>
                                        </tr>
                                    </c:forEach>
                                </tbody>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary" id="addNew" data-controller="usuarios" data-url="./vocales">Guardar</button>
                    </div>
                </div><!-- /.modal-content -->
            </div><!-- /.modal-dialog -->
        </div>
